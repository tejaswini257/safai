...
const LocationAssignmentCard = ({ location }) => (
  <div className="border border-gray-100 p-4 rounded-xl flex flex-col justify-between bg-white shadow-sm hover:shadow-md transition-shadow">
    ...
    <p className="text-xs text-gray-500 mb-2">
      <span className="block text-gray-700 font-medium">Nagpur, Maharashtra - 839292</span>
    ...
    <div className="text-xs text-gray-500 mt-2 pt-2 border-t border-gray-100">
      <span className="flex items-center">
        ...
        Assigned on {location.assignedDate}
      </span>
    </div>
  </div>
);
...
const UserDetailCard = ({ user, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-300 bg-opacity-75 flex items-start justify-center overflow-y-auto z-50 py-10">
          ...
          {totalLocations > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {user.locations.map((location, index) => (
                <LocationAssignmentCard key={index} location={location} />
              ))}
            </div>
          ) : (
...
const UserList = () => {
  ...
  const handleAddUser = (newUser) => {
    ...
    setUsers([newUserWithDefaults, ...users]);
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="mb-6">
        <div className="relative">
          ...
          <input
            type="text"
            className="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg bg-white shadow-sm focus:ring-teal-500 focus:border-teal-500 sm:text-sm"
