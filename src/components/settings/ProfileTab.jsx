...
export default function ProfileTab({ onNotify }) {
    ...
    const save = () => {
        setLoading(true);
        setTimeout(() => { setLoading(false); onNotify("Profile Updated!"); }, 1000);
    };
            ...
            <div className="flex items-center gap-6">
                <div className="relative group">
                    <div className="h-24 w-24 rounded-full border-4 border-[#E0F7FA] overflow-hidden bg-slate-100">
                        <img src="/image/dashboard img.png" alt="Avatar" className="h-full w-full object-cover" />
                    </div>
                    <button className="absolute bottom-0 right-0 p-2 bg-[#007C85] text-white rounded-full border-2 border-white hover:bg-[#58BECF]">
